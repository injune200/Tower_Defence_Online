export class Tower {
  constructor(x, y) {
    ...
    this.width = 60; // 타워 이미지 가로 길이 (이미지 파일 길이에 따라 변경 필요하며 세로 길이와 비율을 맞춰주셔야 합니다!)
  ...
  draw(ctx, towerImages) {
    ctx.drawImage(towerImages[this.level - 1], this.x, this.y, this.width, this.height);
  ...
  attack(monster, opponentPowerOverwhelming = false) {
    // 타워가 타워 사정거리 내에 있는 몬스터를 공격하는 메소드이며 사정거리에 닿는지 여부는 game.js에서 확인합니다.
    if (this.cooldown <= 0) {
      monster.hp -= opponentPowerOverwhelming ? 0 : this.attackPower * this.level;
